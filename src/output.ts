...
const CATEGORY_CONFIG: Record<PRCategory, { icon: string; label: string; color: (s: string) => string }> = {
  "needs-re-review": { icon: "◆", label: "Needs Re-review", color: chalk.yellow },
  requested: { icon: "●", label: "Requested Reviews", color: chalk.green },
  stale: { icon: "○", label: "Stale", color: chalk.red },
  "waiting-on-others": { icon: "◇", label: "Your PRs Waiting", color: chalk.dim },
};

const CATEGORY_ORDER: PRCategory[] = ["needs-re-review", "requested", "stale", "waiting-on-others"];
...
export function formatStatus(result: StatusResult): string {
  ...
  for (const category of CATEGORY_ORDER) {
    ...
    lines.push("");
    lines.push(` ${config.color(`${config.icon} ${config.label}`)} ${chalk.dim(`(${prs.length})`)}`);
  ...
  if (total > 0) {
    lines.push(chalk.dim(` ${total} PR${total === 1 ? "" : "s"} need attention across ${repos} repo${repos === 1 ? "" : "s"}`));
  }
